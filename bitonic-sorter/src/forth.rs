              ...
              F: Sync + Fn(&T, &T) -> Ordering
{
    if x.len() > 1 {
        ...
        // x を mid_point を境にした2つの可変の借用に分割し、 first と second に束縛する
        let (first, second) = x.split_at_mut(mid_point);

        // x の分割後の要素数をしきい値 ( PARALLEL_THRESHOLD ) と比較する
...
mod tests{
    ...
    {
        {
            // 乱数で 65,536 要素のデータを作成する( 65,536 は 2 の 16乗 )
